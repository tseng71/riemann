...
reindexing preserves the full spectral defect.  Each full consecutive
267-point principal block and its normalized adjacent gaps are now concrete
Lean objects, with positivity and telescoping spans checked.  The 267
concrete shifted partitions, their endpoint-remainder fibers, global
pinching, and insertion into the exact finite simple-zero count are also
checked.  It does not yet replay the Arb certificate or close endpoint
deletion, the summably uniform Gram-to-kernel comparison, and the asymptotic
top-level specialization interfaces.
...
its exact defect invariance, and specializes the local block bridge to every
concrete consecutive 267-point principal block.  It also realizes all 267
shifted partitions and endpoint fibers on the actual ordered index, proves
their global pinching bound, and inserts the resulting finite defect bound
into the exact simple-zero count.  It leaves the local certificate replay,
endpoint deletion, summably uniform Gram-to-kernel error, and asymptotic
top-level specialization as explicit open obligations.  Subject to those stated
...
abstract Theorem~D source inequality, concrete shifted partitions with
endpoint fibers, their finite simple-count assembly, and exact algebra, but
does not yet prove the local seven-point claim or the strengthened
end-to-end top-level statement.
...
upstream concrete parameter family, as well as endpoint deletion and the
summably uniform Gram-to-kernel replacement in the remaining analytic
bridge.
